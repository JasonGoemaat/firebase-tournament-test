...
const createDefaultConfig = () : TournamentConfig => {
  const config: TournamentConfig = {
    ...
    games: [
      ...
      { spotA: 16, spotB: 17, winnerTo: 33 },
      { spotA: 18, spotB: 19, winnerTo: 35 },
      { spotA: 20, spotB: 21, winnerTo: 37 },
      { spotA: 22, spotB: 23, winnerTo: 39 },

      // winner bracket round 2 (games 12-15)
      { spotA: 24, spotB: 25, winnerTo: 40, loserTo: 32},
      { spotA: 26, spotB: 27, winnerTo: 41, loserTo: 34},
      { spotA: 28, spotB: 29, winnerTo: 42, loserTo: 36},
      { spotA: 30, spotB: 31, winnerTo: 43, loserTo: 38},

      // loser bracket round 2 (games 16-19)
      { spotA: 32, spotB: 33, winnerTo: 44},
      { spotA: 34, spotB: 35, winnerTo: 45},
      { spotA: 36, spotB: 37, winnerTo: 46},
      { spotA: 38, spotB: 39, winnerTo: 47},

      // winner bracket round 3 (games 20-21)
      { spotA: 40, spotB: 41, winnerTo: 52, loserTo: 48},
      { spotA: 42, spotB: 43, winnerTo: 53, loserTo: 51},

      // loser bracket round 3 (games 22-23) (losers go to 7th and 8th place)
      { spotA: 44, spotB: 45, winnerTo: 49, loserTo: 71},
      { spotA: 46, spotB: 47, winnerTo: 50, loserTo: 72},

      // loser bracket round 4 (games 24-25) (losers go to 5th and 6th place)
      { spotA: 48, spotB: 49, winnerTo: 54, loserTo: 67},
      { spotA: 50, spotB: 51, winnerTo: 55, loserTo: 68},

      // winner bracket round 4
      { spotA: 52, spotB: 53, winnerTo: 58, loserTo: 56},

      // loser bracket round 5
      { spotA: 54, spotB: 55, winnerTo: 57, loserTo: 66},

      // loser bracket round 6
      { spotA: 56, spotB: 57, winnerTo: 59, loserTo: 65},

      // finals - winners of winner bracket and lose bracket
      { spotA: 58, spotB: 59, winnerTo: 60, loserTo: 61},

      // final playoff (if needed because loser bracket champion beat winner bracket champion, if
      // not needed just select the winner)
      { spotA: 60, spotB: 61, winnerTo: 62, loserTo: 64},

      // playoff to determine 5th and 6th
      { spotA: 67, spotB: 68, winnerTo: 69, loserTo: 70},

      // playoff to determine 7th and 8th
      { spotA: 71, spotB: 72, winnerTo: 73, loserTo: 74},
    ]
  };
